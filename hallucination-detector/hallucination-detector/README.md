...
```
├── DESIGN.md                     # system design + architecture diagram
├── configs/default.yaml          # config knobs
├── scripts/
│   ├── train.py                  # end-to-end training CLI
│   └── detect.py                 # inference / demo CLI
├── src/halludetect/
│   ├── data/                     # schema, synthetic gen, RAGTruth curation, splits
│   ├── features/extractor.py     # HF + mock hidden-state extraction (per-layer pool)
│   ├── probe/                    # linear probe + per-layer sweep / selection
│   ├── eval/evaluate.py          # macro-F1, per-class report, confusion, cost stub
│   ├── serve/detector.py         # load probe -> detect()
│   └── pipeline.py               # orchestrates the whole run
├── tests/test_pipeline.py        # 8 tests, all on the mock backend
└── outputs/                      # probe.joblib + report.json (after training)
```
